...
- **Task Status**: Each task is `Active`, `Estimated` or `Inactive`, set from the editor's Status dropdown and defaulting to `Active`. Active is the ordinary case and leaves the list's Status column blank; `Estimated` shows a bold **E** and `Inactive` a bold **I**, and an Inactive row is struck through and dimmed to grey so a task set aside reads as set aside. The dashboard's summary counts the three as shares that come to a hundred — Active, Estimated (E) and Inactive (I). A plan written by an earlier version, whose tasks carried the old `Draft` status, opens with those rows read as `Active`
