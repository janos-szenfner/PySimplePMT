...
======================
Each level of the plan counts what is under it differently - a Task averages
its sub-tasks' percentages, a Phase averages its tasks evenly, and anything
else weights its children by how long they run - and which rule is applied to
what is the sort of thing that is quietly wrong for a long time. The rules are set
...
class TestAnythingElseWeightsByDuration(unittest.TestCase):
    """
    A long child counts for more of its parent than a short one.

    This was the Deliverable's rule, and Deliverable is no longer a type a
    plan can hold. The rule stays as the answer for anything that comes to
    have children and is neither a Phase nor a Task - a plan can arrive from
    a file holding whatever its own format allowed, and a rule that covers
    it beats a parent that totals to nothing.
    """

    def test_it_weights_by_days(self):
        """Ten finished days of thirty is a third of the way through."""
        parent = task("D", "Workstream")
    ...
    def test_zero_length_children_are_averaged_instead(self):
        ...
        A parent holding only milestones has no days in it, and dividing
        by that total would be dividing by nothing.
        """
        parent = task("D", "Workstream")
...
class TestAPhaseAveragesItsTasks(unittest.TestCase):
    """Tasks count evenly towards the phase holding them."""

    def test_it_averages_them(self):
        """Two tasks, one done, is half the phase."""
        parent = task("P", "Phase")
        children = [task("T1", "Task", progress=100, days=1),
                    task("T2", "Task", progress=0, days=1)]

        self.assertEqual(rolled_up_progress(parent, children), 50)

    def test_length_does_not_come_into_it(self):
        """
        A longer task is not a larger share of the phase.

        Tasks are the units a phase is scoped in; the same two at 100% and
        0% make half a phase however long either runs.
        """
        parent = task("P", "Phase")
        children = [task("T1", "Task", progress=100, days=2),
                    task("T2", "Task", progress=0, days=200)]
...
class TestEmptyAndOutOfRange(unittest.TestCase):
    ...
    def test_a_container_with_nothing_in_it_is_nothing_done(self):
        ...
        self.assertEqual(rolled_up_progress(task("D", "Workstream"), []), 0)
...
class TestTheWholeCascade(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.project.add_task(task("T1", "Task", parent="P", days=10,
                                   start=base))
        ...
        self.project.add_task(task("T2", "Task", parent="P", days=10,
    ...
    def test_a_ticked_subtask_reaches_the_phase(self):
        ...
        self.assertEqual(self.of("T1"), 50)     # one of its two sub-tasks
        self.assertEqual(self.of("T2"), 40)     # its own percentage
